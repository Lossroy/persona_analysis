...
# Alternatif Yöntem:
df.pivot_table("PRICE", ["SEX", "SOURCE"], ["COUNTRY", "AGE"], aggfunc="mean")
...
agg_df1 = df.groupby(["COUNTRY", "SOURCE", "SEX", "AGE"]).agg({"PRICE": "mean"}).sort_values("PRICE", ascending=False)
agg_df2 = df.groupby(["COUNTRY", "SOURCE", "SEX", "AGE"]).agg({"PRICE": "mean"}).sort_values(ascending=False, by="PRICE")
...
agg_df["AGE_CAT"] = pd.cut(agg_df["AGE"], [0, 18, 23, 30, 40, 70], labels=['kid', 'young', 'adult', 'mature', 'old'])
agg_df["AGE"].nunique()
...
agg_df["customers_level_based"] = (agg_df['COUNTRY'].str.upper() + "_" + agg_df['SOURCE'].str.upper() + "_"
                                  + agg_df['SEX'].str.upper() + "_" + agg_df['AGE_CAT'].str.upper())
...
agg_df["COUNTRY"] + agg_df["AGE_CAT"]
agg_df["COUNTRY"] + agg_df["SOURCE"]
...
agg_df.loc[agg_df["customers_level_based"] == new_user1]
